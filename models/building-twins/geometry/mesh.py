...
alpha1 = np.arctan(h1/((b1-w1)/2)); beta1 = np.pi/4-alpha1/2;
alpha2 = np.arctan(h2/((b2-w2)/2)); beta2 = np.pi/4-alpha2/2;
alpha3 = np.arctan(h3/((b3-w3)/2)); beta3 = np.pi/4-alpha3/2;
...
def geometry1_unstructured(output_file, lc_wall=0.01, lc_bulk=0.1):
    ...
    gmsh.model.occ.addCurveLoop([1000, 
                                 1002, 1012, 
                                 1102, 
                                 1202, 1212, 
                                 1302, 1312, 1322, 
...
def main(lc_wall=.1, lc_bulk=1, type="unstructured", output_file="g1.msh"):
    if type == "structured":
